...
class AllProcessor(DataProcessor):
    """Processor for the combined data set: MNLI, MRPC, SICK, SNLI, STS-B"""
...
def do_validate(model, processor, tokenizer, device, args):
...
def do_evaluate_sample(model, sent_a, sent_b, tokenizer, max_seq_length, device):
    """
    Returns:
        similarity: float
    """
    ...
    # concat two senteces; this is the same of convert_examples_to_features()
    tokens = []
    segment_ids = []
    tokens.append("[CLS]")
    segment_ids.append(0)
    ...
    logits = model(input_ids_tensor, segment_ids_tensor, input_mask_tensor)
...
def main():
    ...
    parser.add_argument("--do_evaluate_sample",
                        default=False,
                        action='store_true')
    ...
    parser.add_argument('--early_training_epoch_threshold',
                        type=int, default=20)
        ...
        for _ in range(int(args.num_train_epochs)):
            ...
            for step, batch in enumerate(train_dataloader):
                ...
                if (step + 1) % args.gradient_accumulation_steps == 0:
                    ...
                    model.zero_grad()
                    global_step += 1
                if (step + 1) % args.log_step == 0:   # log step
                    logger.info("%s - Epoch [%s/%s], Step [%s/%s], Loss: %.4f" % (time.ctime(), _+1, args.num_train_epochs, step+1, 
                        len(train_dataloader), loss.item()))

            # After each epoch, check whether save model
            model.eval()
